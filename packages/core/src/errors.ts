...
export class SkillValidationError extends USKError {
  constructor(
    ...
    super(
      `Skill validation failed: ${validationErrors.length} error(s)`,
...
export class ConversionError extends USKError {
  constructor(skillPath: string, reason: string, cause?: Error) {
    super(
      `Conversion failed for ${skillPath}: ${reason}`,
      'CONVERSION_ERROR',
...
export function getErrorSuggestions(error: unknown): string[] {
  ...
  switch (error.code) {
    case 'SKILL_NOT_FOUND':
      suggestions.push('确保文件路径正确')
      suggestions.push('如果是目录，请确保其中包含 SKILL.md 文件')
      suggestions.push('使用绝对路径或相对于当前工作目录的路径')
      break

    case 'SKILL_VALIDATION_ERROR':
      suggestions.push('运行 `usk analyze <skill>` 查看详细验证信息')
      suggestions.push('修复所有错误后重新转换')
      suggestions.push('或使用 --interactive 模式手动确认继续')
      break

    case 'CONVERSION_ERROR':
      suggestions.push('检查目标平台是否正确 (claude/codex)')
      suggestions.push('确保输出目录有写入权限')
      suggestions.push('使用 --verbose 查看详细错误信息')
      break

    case 'RESOURCE_NOT_FOUND':
      suggestions.push('检查 Skill 中引用的资源文件路径是否正确')
      suggestions.push('确保所有引用的文件都存在')
      suggestions.push('使用相对路径而不是绝对路径')
      break

    case 'FILE_WRITE_ERROR':
      suggestions.push('检查输出目录是否存在写入权限')
      suggestions.push('确保输出路径有效')
